...
--                     Snd beh' _ -> discretize (Beh (Fun _ f ::: beh'))
...
zipWith f (Beh (x ::: xs)) (Beh (y ::: ys)) =
  ...
  where
    ...
    app (Fun s x') (Fun s' y') =
      Fun (s :* s') -- TODO: Look into this
        ( box
            ( \t ->
                let (a :* ab :* s'') = unbox x' t
                    (b :* bb :* s''') = unbox y' t
                    left = unbox f a b
                 in left :* (ab || bb) :* (s'' :* s''')
            )
...
--     run (Fun s f ::: xs) = Fun s (box (\t -> let (a :* b) = unbox f t in (a :* (unbox p a || b)))) ::: delay (run (adv xs))
